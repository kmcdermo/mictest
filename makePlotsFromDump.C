void makePlotsFromDump(TString test, bool isCMSSW = false, bool isEndcap = false)
{
  ...
  TFile* f1 = TFile::Open("test_snb_"+events+"_"+test+"_NVU1_NTH12.root");
  TH1F* h1 = (TH1F*) f1->Get("h_MXNH");
  h1->SetLineColor(kBlack);
  h1->SetLineWidth(2);
  if (test== "BH") h1->SetTitle("Best Hit");
  if (test== "COMB") h1->SetTitle("Combinatorial");
  ...
  TFile* f3 = TFile::Open("test_knc_"+events+"_"+test+"_NVU1_NTH12.root");
  ...
  TLegend* leg = new TLegend(0.75,0.65,1.0,0.88);
  leg->SetBorderSize(0);
  leg->AddEntry(h1,"SNB NVU1 NTH12","L");
  leg->AddEntry(h2,"SNB NVU8 NTH24","LP");
  leg->AddEntry(h3,"KNC NVU1 NTH60","LP");
