...
namespace Config{
  Double_t range = 0.01;
  Double_t fitrange = range*0.8;
  TString  formname = "gaus3fm";
  Int_t    nbins = 50;
  TString  histname;
  TString  var   = "phi";
  Int_t    layer = 16;
  TString  outdir = "zphicmssw";
};
...
void tuning()
{
  ...
  // prep hist
  TFile * file = TFile::Open("valtree_mateffects.root");
  TTree * tree = (TTree*)file->Get("fittree");

  TH1F * hist = new TH1F(Form("h_%s_diff_lay%i",Config::var.Data(),Config::layer),
			 Form("%s_prop[%i]-%s_hit[%i]",Config::var.Data(),Config::layer,Config::var.Data(),Config::layer),
			 Config::nbins,-Config::range,Config::range); 
  Config::histname = hist->GetName();
  hist->Sumw2();

  // fill the hist
  tree->Draw(Form("%s_prop[%i]-%s_hit[%i]>>%s",Config::var.Data(),Config::layer,Config::var.Data(),Config::layer,Config::histname.Data()),
	     Form("%s_hit[%i]!=-1000",Config::var.Data(),Config::layer),"goff");
...
void fittingCore(TH1F *& hist)
{
  ...
  // draw fit + subcomponents
  TF1 * sub1; TF1 * sub2; TF1 * sub3;
  drawFit(fit,lincanv,sub1,sub2,sub3);

  // linear canvas only has mean, sigma, and chi2
  ...
  // save linear canvas
  lincanv->SaveAs(Form("%s/%s_%s_lin.pdf",Config::outdir.Data(),Config::histname.Data(),Config::formname.Data()));
  ...
  // draw individual params
  TPaveText * stats;
  drawStats(fit,stats,logcanv);

  logcanv->SaveAs(Form("%s/%s_%s_log.pdf",Config::outdir.Data(),Config::histname.Data(),Config::formname.Data()));
...
{
  TF1 * tempfit = new TF1("temp","gaus(0)",-Config::fitrange*0.5,Config::fitrange*0.5);
  ...
  if (Config::formname.EqualTo("gaus1",TString::kExact)) {
    ...
    fit->SetParName(1,"mean");   fit->SetParameter(1,mean);
    fit->SetParName(2,"sigma1"); fit->SetParameter(2,sigma*0.7); fit->SetParLimits(2,0,10); //fit->SetParLimits(2,sigma*0.5,sigma);
    fit->SetParName(3,"norm2");  fit->SetParameter(3,norm*0.3);  //fit->SetParLimits(3,norm*0.1,norm*0.5);
    fit->SetParName(4,"sigma2"); fit->SetParameter(4,sigma*1.4); fit->SetParLimits(4,0,10); //fit->SetParLimits(4,sigma,sigma*1.5);
    fit->SetParName(5,"norm3");  fit->SetParameter(5,norm*0.01); //fit->SetParLimits(5,norm*0.005,norm*0.1);
    fit->SetParName(6,"sigma3"); fit->SetParameter(6,sigma*2.5); fit->SetParLimits(6,0,10); //fit->SetParLimits(6,sigma*1.5,sigma*5.0);
  }
...
{
  delete fit;
  if (Config::formname.EqualTo("gaus2fm",TString::kExact) || Config::formname.EqualTo("gaus3fm",TString::kExact))
  {
    delete sub1;
    delete sub2;
    if (Config::formname.EqualTo("gaus3fm",TString::kExact)) 
