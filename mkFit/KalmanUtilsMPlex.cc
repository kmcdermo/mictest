                           ...
                           const int      N_proc)
{
  ...
  // Tangent point is half way between hit and psagate position
...
#pragma simd
  ...
  MPlexLV psPar_ccs;// psagated parameters in CCS coordinates
  MPlexLL jac_ccs;    // jacobian from cartesian to CCS
  ConvertToCCS(psPar,psPar_ccs,jac_ccs);

  MPlexLL tempLL;
  CCSErr      (jac_ccs, psErr, tempLL);
  CCSErrTransp(jac_ccs, tempLL, psErr);// psErr is now psagated errors in CCS coordinates
#endif
