...
echo "Writing 10k tracks/event * 20 events for ToyMC barrel-only building tests"
./mkFit/mkFit --num-thr-sim 1 --write --file-name simtracks_barrel_20x10k.bin
mv simtracks_barrel_20x10k.bin ${dir}/

# Fitting test: Barrel
echo "Writing 10k tracks/event * 1k events for ToyMC barrel-only fiting tests"
./mkFit/mkFit --num-thr-sim 1 --write --num-events 1000 --file-name simtracks_barrel_1kx10k.bin
mv simtracks_barrel_1kx10k.bin ${dir}/

# Fitting test: Endcap
echo "Writing 10k tracks/event * 1k events for ToyMC endcap-only fiting tests"
...
echo "Writing 10k tracks/event * 10 events for ToyMC barrel-only validation tests"
sed -i 's/#WITH_ROOT := yes/WITH_ROOT := yes/g' Makefile.config
./mkFit/mkFit --normal-val --num-thr-sim 1 --num-events 10 --write --file-name simtracks_barrel_10x10k_val.bin
