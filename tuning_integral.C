...
void tuning_integral()
{
  ...
  bool save = true;
  ofstream yields(Form("zphi/%sintegrals.txt",var.Data()),std::ios_base::app);
  ...
  tree->Draw(Form("(%s_prop[%i]-%s_hit[%i])/sqrt(e%s%s_prop[%i])>>%s",var.Data(),layer,var.Data(),layer,var.Data(),var.Data(),layer,hist->GetName()),Form("%s_hit[%i]!=-1000",var.Data(),layer),"goff");
