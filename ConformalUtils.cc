...
void rotateBack(float& x, float& y, const float delphi){
  // perform rotation
  const float xprime =  x*cos(delphi)+y*sin(delphi);
  const float yprime = -x*sin(delphi)+y*cos(delphi);
...
void conformalFit(const Hit& hit0, const Hit& hit1, const Hit& hit2, int charge, TrackState& fitStateHit0, bool fiterrs) {
  ...
  float initphi = fabs(atan2(y[1],x[1]));

  if (initphi < Config::PIOver4 || initphi > Config::PI3Over4){


  //  std::cout << "x[0]: " << x[0] << " y[0]: " << y[0] << std::endl;

//   const float delphi = Config::PIOver4 - std::atan2(y[1],x[1]);
//   rotateHitPos(x[0],y[0],delphi);
//   rotateHitPos(x[1],y[1],delphi);
//   rotateHitPos(x[2],y[2],delphi);
  ...
  for (unsigned int h=0;h<3;++h) {
    ...
    // new coords
    //u[h]=y[h]/(x[h]*x[h]+y[h]*y[h]);
    //    v[h]=x[h]/(x[h]*x[h]+y[h]*y[h]);
  }
  ...
  // new coords
  //float R=sqrt((x[0]-b)*(x[0]-b)+(y[0]-a)*(y[0]-a));
  ...
  */
  ...
  // taking vector from center of circle to layer one point
  // therefore phi is the perpendicular to vector just described

   float vrx = x[0]-a;
   float vry = y[0]-b;

//   rotateBack(x[0],y[0],delphi);
//   rotateBack(x[1],y[1],delphi);
//   rotateBack(x[2],y[2],delphi);

//   rotateBack(vrx,vry,delphi);
...
//  float vry = y[0]-a;
  ...
  //std::cout << "pos_mc[0] phi: " << atan2(y[0],x[0]) << " phi_cf: " << atan2(py,px) << std::endl;  
