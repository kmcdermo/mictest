...
do
    ...
    ssh mic0 ./mkFit-mic --read --file-name ${micdir}/simtracks_barrel_20x10k.bin --build-bh  --num-thr 60 >& log_knc_20x10k_BH_NVU${nvu}_NTH60.txt
    echo "knc toymc" nvu=${nvu} "COMB (barrel)"
    ssh mic0 ./mkFit-mic --read --file-name ${micdir}/simtracks_barrel_20x10k.bin --build-comb --seeds-per-task 32 --num-thr 60 --cloner-single-thread >& log_knc_20x10k_COMB_NVU${nvu}_NTH60.txt
