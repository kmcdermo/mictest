...
void makeBenchmarkPlotsSeed()
{
  ...
  TFile* f = TFile::Open("benchmark_seeding.root");
  ...
  for (UInt_t i = 0; i < tests.size(); i++)
  {
    ...
    g_1000_VU->SetTitle(Form("%s Vectorization Benchmark [nTH="+nth+"]",stests[i].Data()));
    ...
    g_1000_VU_speedup->SetTitle(Form("%s Vectorization Speedup [nTH="+nth+"]",stests[i].Data()));
    ...
    c2.SetGridy();
    c2.Update();
    c2.SaveAs(Form("%s_vu_speedup.png",tests[i].Data()));
    ...
    g_1000_TH->SetTitle(Form("%s Parallelization Benchmark [nVU="+nvu+"]",stests[i].Data()));
    ...
    c3.SetLogy();
    c3.Update();
    c3.SaveAs(Form("%s_th_time.png",tests[i].Data()));
    delete leg_TH;
    ...
    g_1000_TH_speedup->SetTitle(Form("%s Parallelization Speedup [nVU="+nvu+"]",stests[i].Data()));
